...
<li class="nav-item dropdown">
 ...
 <li><a class="dropdown-item" href="tripDetails.php">Trip Details</a></li>
    ...
    $conn = oci_connect($user, $password, $connectionString);
            ...
            $sql = "SELECT * FROM DRIVER WHERE UNIT_NAME = '$username' AND (UPPER(DRIVER_ID) LIKE '%$searchTerm%' OR UPPER(DRIVER_NAME) LIKE '%$searchTerm%' OR UPPER(QUALIFICATION) LIKE '%$searchTerm%' OR UPPER(AVAILABILITY) LIKE '%$searchTerm%')";
        } 
            else {
            // Display the data for the user who created the session
            $sql = "SELECT * FROM DRIVER WHERE UNIT_NAME = '$username'";
            }
            ...
            while ($row = oci_fetch_assoc($stmt)) {
                ...
                echo "<td class='" . ($row['AVAILABILITY'] == 'UNAVAILABLE' ? 'unavailable' : '') . "'>" . $row['AVAILABILITY'] . "</td>";
