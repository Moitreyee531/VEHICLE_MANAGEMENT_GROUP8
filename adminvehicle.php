...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['search'])) {
    ...
    $sql = "SELECT VEHICLE_ID, TYPE_OF_VEHICLE, KMPL, DATE_OF_INDUCTION, CLASSIFICATION, OWNER_UNIT, SERVICE_STATUS, (SYSDATE - DATE_OF_INDUCTION) AS SERVICE_LENGTH FROM VEHICLE WHERE (UPPER(VEHICLE_ID) LIKE '%$searchTerm%' OR UPPER(TYPE_OF_VEHICLE) LIKE '%$searchTerm%' OR UPPER(CLASSIFICATION) LIKE '%$searchTerm%' OR UPPER(KMPL) LIKE '%$searchTerm%' OR UPPER(DATE_OF_INDUCTION) LIKE '%$searchTerm%')";
} else {
    // Display the default query when the page is refreshed or no search term is provided
    $sql = "SELECT VEHICLE_ID, TYPE_OF_VEHICLE, KMPL, DATE_OF_INDUCTION, CLASSIFICATION, OWNER_UNIT, SERVICE_STATUS, (SYSDATE - DATE_OF_INDUCTION) AS SERVICE_LENGTH FROM VEHICLE ";
}
