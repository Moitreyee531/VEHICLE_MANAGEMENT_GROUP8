...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $deleteVehicleSql = "DELETE FROM VEHICLE WHERE OWNER_UNIT= upper(:username) and  VEHICLE_ID = upper(:vehicleID)";
    $deleteVehicleStmt = oci_parse($conn, $deleteVehicleSql);
    oci_bind_by_name($deleteVehicleStmt, ':vehicleID', $vehicleID);
    oci_bind_by_name($deleteVehicleStmt, ':username', $username);


    // Execute the delete statement
    $deleteResult = oci_execute($deleteVehicleStmt);

    // Check if the delete operation was successful
    if ($deleteResult) {
        // Redirect to driverDetails.php after a short delay
        header("refresh:3;url=vehicleDetails.php");
    } else {
        $error = oci_error($deleteVehicleStmt);
        echo '<div class="alert alert-danger mt-4">Failed to delete driver entry: ' . $error['message'] . '</div>';
    }

    // Free the statement and close the connection
    oci_free_statement($deleteVehicleStmt);
    oci_close($conn);
}
?>
...
<div class="container">
    <div class="row">
        <div class="col-md-6 offset-md-3">
            <h3 class="text-center mt-4">Delete Vehicle</h3>
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                <div class="mb-3">
                    <label for="DriverID" class="form-label">Vehicle ID</label>
