...
<li class="nav-item dropdown">
 ...
 <li><a class="dropdown-item" href="tripDetails.php">Trip Details</a></li>
    ...
    <table class="table table-striped table-bordered" style="margin-top: 20px;">
        ...
        <?php
            ...
            $sql1 = 'SELECT UNIQUE vehicle.vehicle_id, type_of_vehicle, classification, KMPL, SUM(trip.fuel_consumption) AS total_fuel_consumption FROM vehicle LEFT OUTER JOIN trip ON vehicle.vehicle_id = trip.vehicle_id  WHERE VEHICLE.OWNER_UNIT = :username AND VEHICLE.SERVICE_STATUS = :serviceStatus  GROUP BY vehicle.vehicle_id, type_of_vehicle, classification, KMPL, trip.fuel_consumption';
    ...
    <h4 style="text-align: center; color: black;">On Trip Vehicle</h4>
    <table class="table table-striped table-bordered" style="margin-top: 20px;">
        <thead class="table-dark">
            <tr>
                <th>Vehicle ID</th>
        ...
        <?php
            ...
            WHERE vehicle.owner_unit = :username AND vehicle.service_status = :serviceStatus2
 ...
 $sql2 = "SELECT DRIVER.DRIVER_ID, DRIVER.DRIVER_NAME, DRIVER.QUALIFICATION, DRIVER.AVAILABILITY, DRIVER.UNIT_NAME, LEAVE.LEAVE_DUE FROM DRIVER JOIN LEAVE ON DRIVER.DRIVER_ID = LEAVE.DRIVER_ID WHERE DRIVER.UNIT_NAME = :username AND LEAVE.LEAVE_DUE >= TO_DATE('" . date('Y-m-d') . "', 'YYYY-MM-DD')  ORDER BY LEAVE.LEAVE_DUE ASC";

 // Prepare the statement
 $stmnt = oci_parse($conn, $sql2);
 oci_bind_by_name($stmnt, ":username", $username);
