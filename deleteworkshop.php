...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Retrieve the workshop ID from the form submission
    ...
    $deleteWorkshopSql = "DELETE FROM WORKSHOP WHERE VEHICLE_ID = :vehicleID";
    $deleteWorkshopStmt = oci_parse($conn, $deleteWorkshopSql);
    oci_bind_by_name($deleteWorkshopStmt, ':vehicleID', $vehicleID);

    // Execute the delete statement
    $deleteResult = oci_execute($deleteWorkshopStmt);

    if ($deleteResult) {
        $rowsAffected = oci_num_rows($deleteWorkshopStmt);

        if ($rowsAffected > 0) {
            // Delete successful
            oci_commit($conn); // Commit the transaction
            oci_free_statement($deleteWorkshopStmt);
            oci_close($conn);

            // Redirect to the workshop.php page after a successful delete
            ...
            oci_free_statement($deleteWorkshopStmt);
            oci_close($conn);
            echo "No workshop record found for the specified workshop ID.";
        }
    } else {
        // Delete statement failed
        $error = oci_error($deleteWorkshopStmt);
        oci_rollback($conn); // Rollback the transaction
        oci_free_statement($deleteWorkshopStmt);
        oci_close($conn);
        die("Failed to delete workshop: " . $error['message']);
    }
}
?>
...
<div class="text-center" style="margin-bottom: 20px;">
    ...
    <a class="btn btn-primary d-inline-block" href="workshop.php">Next</a>
</div>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Create a connection to the Oracle database
    $conn = oci_connect($user, $password, "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=$host)(PORT=$port))(CONNECT_DATA=(SERVICE_NAME=$service)))");
